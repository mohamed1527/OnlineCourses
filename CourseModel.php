...
class Course extends Model{ 
    ...
    function __construct($sections="",$teacher="",$coursename="",$courseid="",$coursetype="",$courseimage="",$startdate="",$enddate="") {
          ...
          function studentcourses($sid,$cid){
            $sql = "INSERT into studentcourse(sid,cid) values ($sid,$cid);";
            $dbh = new Dbh();
            $result = $dbh->query($sql);

            if($result == TRUE){
              echo "Students Added Successfully to Courses";
            }
            else{
         ...
         public function getcoursesusers($sid){
            $sql = "SELECT * FROM studentcourse WHERE sid=$sid ";
            $dbh = new Dbh();
            $result = $dbh->query($sql);

            $students= array();
            while($row=mysqli_fetch_assoc($result)){
              $sql = "SELECT * FROM Student where ID=".$row['sid'];
              $dbh = new Dbh();
              $result= $dbh->query($sql);
